    ...
    <el-card class="per-container">
      <div class="per-content">
        ...
        >
          ...
          ></el-avatar>
          <img v-else class="avatar-img" :src="editForm.avatar_url" />
        </el-upload>
        <span class="username">{{
          editForm.nick_name ? editForm.nick_name : '无'
        }}</span>
    ...
    <el-card class="per-message">
      ...
      >
        <el-form-item label="用户信息" prop="name">
          ...
          <el-input v-else v-model:value="editForm.name"></el-input>
        </el-form-item>
        <el-form-item label="昵称" prop="nick_name">
          <span class="showmsg" v-if="!editable">{{ editForm.nick_name }}</span>
          <el-input v-else v-model:value="editForm.nick_name"></el-input>
        </el-form-item>
        <el-form-item label="邮箱" prop="email">
          <span class="showmsg" v-if="!editable">{{ editForm.email }}</span>
          <el-input v-else v-model:value="editForm.email"></el-input>
        </el-form-item>
        <el-form-item label="电话号码" prop="phone">
          <span class="showmsg" v-if="!editable">{{ editForm.phone }}</span>
          <el-input v-else v-model:value="editForm.phone"></el-input>
        </el-form-item>
        <el-form-item label="个人简介" prop="personal_info">
          <span class="showmsg" v-if="!editable">{{
          ...
          <el-input
            ...
            :rows="2"
            v-else
            v-model:value="editForm.personal_info"
          ></el-input>
        </el-form-item>
        <el-button
          v-show="!editable"
...
export default {
  ...
  methods: {
    ...
    async getUser() {
      ...
      let { data: res } = await this.$axios.get('/user/user', {
        params: { id: that.uid },
      })
      console.log(res)
