  ...
  <div>
    <el-card>
      ...
      <div class="article-content">
        <el-card class="article" v-for="(a, index) in viewlist" :key="index">
          <h3>{{ a.title }}</h3>
          ...
          <el-dropdown trigger="click" style="margin-left: 20px">
            ...
            <template v-slot:dropdown>
              <el-dropdown-menu>
                ...
                <el-dropdown-item @click="deleteArticle(a)" class="clearfix">
                  删除
                </el-dropdown-item>
              </el-dropdown-menu>
            </template>
...
export default {
  ...
  methods: {
    ...
    async getArticleList() {
      ...
      var type = 'list'
      let { pnum, psize } = this
      let { data: res } = await this.$axios.get('/article', {
        params: {
          uid: uid,
          type: type,
        },
      ...
      if (that.searchContent) {
        ...
        that.viewlist = that.articleList.filter(function (a) {
          ...
          return Object.keys(a).some(function (key) {
            ...
            if (key == 'title')
              return (
                String(a[key]).toLowerCase().indexOf(that.searchContent) > -1
              )
          })
        })
        that.viewlist = that.viewlist.slice(
          (that.pnum - 1) * that.viewlist.length,
          that.psize
        )
    ...
    async deleteArticle(datas) {
      console.log(datas)
      var uid = getUid()
      let { data: res } = await this.$axios.delete('/article', {
        params: {
          uid: uid,
          aid: datas.id,
        },
